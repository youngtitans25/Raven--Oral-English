...
import { ArrowLeft, BookOpen, Globe, Map, Mountain, Users, Flag, Play, Sparkles, ChevronRight, Layers, Compass } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { GEOGRAPHY_SECTIONS, GEOGRAPHY_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface GeographySyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const GeographySyllabusView: React.FC<GeographySyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? GEOGRAPHY_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Explore the <span className="text-green-600">World</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                From map reading to regional studies, master the physical and human aspects of geography.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
            {GEOGRAPHY_SECTIONS.map((section, index) => {
                let Icon = Globe;
                let colorClass = "text-green-600";
                let bgClass = "bg-green-500/5";

                if (section.id.includes('section-1')) { Icon = Map; colorClass = "text-blue-600"; bgClass = "bg-blue-500/5"; }
                if (section.id.includes('section-2')) { Icon = Globe; colorClass = "text-indigo-600"; bgClass = "bg-indigo-500/5"; }
                if (section.id.includes('section-3')) { Icon = Mountain; colorClass = "text-emerald-600"; bgClass = "bg-emerald-500/5"; }
                if (section.id.includes('section-4')) { Icon = Users; colorClass = "text-orange-600"; bgClass = "bg-orange-500/5"; }
                if (section.id.includes('section-5')) { Icon = Flag; colorClass = "text-teal-600"; bgClass = "bg-teal-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    let bgTheme = "bg-green-50";
    
    if (activeSection.id.includes('section-1')) { themeColor = "text-blue-600"; bgTheme = "bg-blue-50"; }
    if (activeSection.id.includes('section-2')) { themeColor = "text-indigo-600"; bgTheme = "bg-indigo-50"; }
    if (activeSection.id.includes('section-3')) { themeColor = "text-emerald-600"; bgTheme = "bg-emerald-50"; }
    if (activeSection.id.includes('section-4')) { themeColor = "text-orange-600"; bgTheme = "bg-orange-50"; }
    if (activeSection.id.includes('section-5')) { themeColor = "text-teal-600"; bgTheme = "bg-teal-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your Geography session.
                </p>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={GEOGRAPHY_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        <Button variant="ghost" onClick={handleBack} className="text-slate-600 gap-2 pl-0 hover:bg-transparent hover:text-green-600 text-sm">
            <ArrowLeft className="w-4 h-4 md:w-5 md:h-5" /> 
            {selectedSectionId ? 'Back to Sections' : 'Back to Dashboard'}
        </Button>
        <div className="flex items-center gap-2 text-slate-800">
            <Layers className="w-4 h-4 md:w-5 md:h-5 text-green-600" />
            <span className="font-display font-bold text-sm md:text-lg">
                Geography <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-green-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-green-50 text-green-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-green-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Comprehensive coverage from Maps to Regional Planning.</span>
                </div>
            </div>
        )}
      </div>
...
export default GeographySyllabusView;
