...
const MusicSyllabusView: React.FC<MusicSyllabusViewProps> = ({ onBack, onStartSection }) => {
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Compose your <span className="text-pink-600">Symphony</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Master Rudiments, Harmony, and History (African, Western & Comparative).
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto">
            {MUSIC_SECTIONS.map((section, index) => {
                let Icon = MusicIcon;
                let colorClass = "text-pink-600";
                let bgClass = "bg-pink-500/5";

                if (section.id.includes('section-b')) { Icon = Sliders; colorClass = "text-violet-600"; bgClass = "bg-violet-500/5"; }
                if (section.id.includes('section-c')) { Icon = Globe; colorClass = "text-amber-600"; bgClass = "bg-amber-500/5"; }
                if (section.id.includes('section-d')) { Icon = Monitor; colorClass = "text-cyan-600"; bgClass = "bg-cyan-500/5"; }
                if (section.id.includes('section-e')) { Icon = Radio; colorClass = "text-indigo-600"; bgClass = "bg-indigo-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    let bgTheme = "bg-pink-50";
    
    if (activeSection.id.includes('section-b')) { themeColor = "text-violet-600"; bgTheme = "bg-violet-50"; }
    if (activeSection.id.includes('section-c')) { themeColor = "text-amber-600"; bgTheme = "bg-amber-50"; }
    if (activeSection.id.includes('section-d')) { themeColor = "text-cyan-600"; bgTheme = "bg-cyan-50"; }
    if (activeSection.id.includes('section-e')) { themeColor = "text-indigo-600"; bgTheme = "bg-indigo-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your Music session.
                </p>
            </div>
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                Music <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-pink-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-pink-50 text-pink-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-pink-100 max-w-full text-left">
                    ...
                    <span>Explore musical history, technology, and creativity.</span>
                </div>
            </div>
        )}
      </div>
...
export default MusicSyllabusView;
