...
import { ArrowLeft, BookOpen, Cross, Scroll, Users, Play, Sparkles, ChevronRight, Layers, Church } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { CRS_SECTIONS, CRS_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface CRSSyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const CRSSyllabusView: React.FC<CRSSyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? CRS_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Explore <span className="text-amber-600">Biblical Themes</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Study the sovereignty of God, the ministry of Jesus, and the growth of the Early Church.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto">
            {CRS_SECTIONS.map((section, index) => {
                let Icon = BookOpen;
                let colorClass = "text-amber-600";
                let bgClass = "bg-amber-500/5";

                if (section.id.includes('section-b')) { Icon = Scroll; colorClass = "text-indigo-600"; bgClass = "bg-indigo-500/5"; }
                if (section.id.includes('section-c')) { Icon = Cross; colorClass = "text-emerald-600"; bgClass = "bg-emerald-500/5"; }
                if (section.id.includes('section-d')) { Icon = Church; colorClass = "text-rose-600"; bgClass = "bg-rose-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    let themeColor = "text-amber-600";
    let bgTheme = "bg-amber-50";
    
    if (activeSection.id.includes('section-b')) { themeColor = "text-indigo-600"; bgTheme = "bg-indigo-50"; }
    if (activeSection.id.includes('section-c')) { themeColor = "text-emerald-600"; bgTheme = "bg-emerald-50"; }
    if (activeSection.id.includes('section-d')) { themeColor = "text-rose-600"; bgTheme = "bg-rose-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your CRS session.
                </p>
            </div>
                    ...
                    >
                        <div className="flex flex-row sm:flex-col h-full text-slate-900">
                             ...
                             <div className="w-1/3 sm:w-full h-auto sm:h-40 overflow-hidden relative shrink-0">
                                <ImageWithFallback 
                                    src={module.image || 'https://images.unsplash.com/photo-1507434965515-61970f2bd7c6?q=80&w=1000&auto=format&fit=crop'} 
                                    alt={module.title}
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={CRS_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        <Button variant="ghost" onClick={handleBack} className="text-slate-600 gap-2 pl-0 hover:bg-transparent hover:text-amber-600 text-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                CRS <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-amber-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-amber-50 text-amber-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-amber-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Deepen your understanding of biblical themes and Christian living.</span>
                </div>
            </div>
        )}
      </div>
...
export default CRSSyllabusView;
