...
const YorubaSyllabusView: React.FC<YorubaSyllabusViewProps> = ({ onBack, onStartSection }) => {
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Ka <span className="text-orange-600">Yoruba</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Master the Yoruba Language, Literature, and Culture.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-5xl mx-auto">
            {YORUBA_SECTIONS.map((section, index) => {
                let Icon = BookOpen;
                let colorClass = "text-orange-600";
                let bgClass = "bg-orange-500/5";
  ...
  const renderDetailView = () => {
    ...
    let themeColor = "text-orange-600";
    let bgTheme = "bg-orange-50";
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your Yoruba session.
                </p>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            <Layers className="w-4 h-4 md:w-5 md:h-5 text-orange-600" />
            <span className="font-display font-bold text-sm md:text-lg">
                Yoruba <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-orange-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-orange-50 text-orange-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-orange-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Comprehensive coverage of Yoruba Language and Culture.</span>
                </div>
            </div>
        )}
      </div>
...
export default YorubaSyllabusView;