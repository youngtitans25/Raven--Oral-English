...
import { GOVERNMENT_SECTIONS, GOVERNMENT_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface GovernmentSyllabusViewProps {
  onBack: () => void;
...
const GovernmentSyllabusView: React.FC<GovernmentSyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? GOVERNMENT_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Governance & <span className="text-blue-600">Polity</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Explore the structures of power, Nigeria's political history, and international relations.
                Select a module to begin.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto">
            {GOVERNMENT_SECTIONS.map((section, index) => {
                let Icon = Landmark;
                let colorClass = "text-blue-600";
                let bgClass = "bg-blue-500/5";

                if (section.id.includes('part-2')) { Icon = Flag; colorClass = "text-green-600"; bgClass = "bg-green-500/5"; }
                if (section.id.includes('part-3')) { Icon = Globe; colorClass = "text-purple-600"; bgClass = "bg-purple-500/5"; }
                if (section.id.includes('part-4')) { Icon = BookOpen; colorClass = "text-orange-600"; bgClass = "bg-orange-500/5"; }
                        ...
                        <Card className="flex-1 flex flex-col md:flex-row overflow-hidden border-0 shadow-lg group-hover:shadow-2xl transition-all duration-300 rounded-3xl h-full">
                            <div className="h-40 md:h-auto md:w-1/3 overflow-hidden relative shrink-0">
                                <ImageWithFallback 
                                    ...
                                    fallbackSrc="https://images.unsplash.com/photo-1541872703-74c5963631df?q=80&w=1000&auto=format&fit=crop"
                                />
  ...
  const renderDetailView = () => {
    ...
    if (activeSection.id.includes('part-2')) { themeColor = "text-green-600"; bgTheme = "bg-green-50"; }
    if (activeSection.id.includes('part-3')) { themeColor = "text-purple-600"; bgTheme = "bg-purple-50"; }
    if (activeSection.id.includes('part-4')) { themeColor = "text-orange-600"; bgTheme = "bg-orange-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your deep dive into Government.
                </p>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={GOVERNMENT_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                Government <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-blue-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-blue-100 max-w-full text-left">
                    ...
                    <span>Comprehensive coverage from Pre-colonial administration to International Relations.</span>
                </div>
            </div>
        )}
      </div>
...
export default GovernmentSyllabusView;
