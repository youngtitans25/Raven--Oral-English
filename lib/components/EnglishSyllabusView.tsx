    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            ...
            <p className="text-slate-600 text-sm md:text-lg">
                The JAMB Use of English exam is divided into three strategic sections. 
                Select a module below to view detailed topics.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {ENGLISH_SECTIONS.map((section, index) => {
                let Icon = BookOpen;
                if (section.id.includes('section-b')) Icon = PenTool;
                if (section.id.includes('section-c')) Icon = Mic;
                ...
                return (
                    ...
                    >
                        <div className="absolute inset-0 bg-emerald-500/5 rounded-3xl transform group-hover:scale-105 transition-transform duration-500 -z-10"></div>
                        ...
                        <Card className="flex-1 flex flex-col overflow-hidden border-0 shadow-lg group-hover:shadow-2xl transition-all duration-300 rounded-3xl h-full">
                            <div className="h-40 md:h-48 overflow-hidden relative">
                                ...
                                />
                                <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 to-transparent flex items-end p-4 md:p-6">
                            ...
                            <div className="p-5 md:p-6 flex flex-col flex-1">
                                ...
                                <div className="flex items-center text-emerald-600 font-bold text-xs md:text-sm group-hover:underline">
    ...
    return (
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                <span className="text-emerald-600 font-bold tracking-widest text-[10px] md:text-xs uppercase bg-emerald-50 px-3 py-1 rounded-full mb-3 inline-block">
                    ...
                    >
                        <div className="flex flex-row sm:flex-col h-full">
                            ...
                            <div className="w-2/3 sm:w-full p-4 md:p-5 flex flex-col justify-center">
                                <h3 className="font-bold text-slate-900 mb-1 text-sm md:text-base group-hover:text-emerald-600 transition-colors line-clamp-2">
                                    {module.title}
                                </h3>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        <Button variant="ghost" onClick={handleBack} className="text-slate-600 gap-2 pl-0 hover:bg-transparent hover:text-emerald-600 text-sm">
            ...
            {selectedSectionId ? 'Back to Syllabus' : 'Back to Dashboard'}
        </Button>
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                Use of English <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-emerald-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-emerald-100 max-w-full text-left">
                    ...
                    <span>Raven adapts to your skill level automatically in every session.</span>
                </div>
            </div>
        )}
      </div>
...
export default EnglishSyllabusView;
