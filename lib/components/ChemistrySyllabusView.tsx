...
import { ArrowLeft, Beaker, Filter, Flame, Zap, Play, Sparkles, ChevronRight, Layers, BookOpen } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { CHEMISTRY_SECTIONS, CHEMISTRY_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface ChemistrySyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const ChemistrySyllabusView: React.FC<ChemistrySyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? CHEMISTRY_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                React with <span className="text-teal-600">Chemistry</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Master the fundamental principles, separation techniques, and chemical changes.
            </p>
        </div>

        <div className="grid grid-cols-1 gap-6 md:gap-8 max-w-3xl mx-auto">
            {CHEMISTRY_SECTIONS.map((section, index) => {
                let Icon = Beaker;
                const colorClass = "text-teal-600";
                const bgClass = "bg-teal-500/5";
  ...
  const renderDetailView = () => {
    ...
    const themeColor = "text-teal-600";
    const bgTheme = "bg-teal-50";
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your Chemistry session.
                </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 md:gap-6">
                {activeSection.subModules?.map((module, index) => {
                    let ModuleIcon = Beaker;
                    if (module.id.includes('separation')) ModuleIcon = Filter;
                    if (module.id.includes('changes')) ModuleIcon = Flame;
                    if (module.id.includes('purity')) ModuleIcon = Zap;
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={CHEMISTRY_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        <Button variant="ghost" onClick={handleBack} className="text-slate-600 gap-2 pl-0 hover:bg-transparent hover:text-teal-600 text-sm">
            <ArrowLeft className="w-4 h-4 md:w-5 md:h-5" /> 
            {selectedSectionId ? 'Back to Topics' : 'Back to Dashboard'}
        </Button>
        <div className="flex items-center gap-2 text-slate-800">
            <Layers className="w-4 h-4 md:w-5 md:h-5 text-teal-600" />
            <span className="font-display font-bold text-sm md:text-lg">
                Chemistry <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-teal-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-teal-50 text-teal-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-teal-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Master the principles of Chemistry.</span>
                </div>
            </div>
        )}
      </div>
...
export default ChemistrySyllabusView;
