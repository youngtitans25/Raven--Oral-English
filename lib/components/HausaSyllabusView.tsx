...
import { ArrowLeft, BookOpen, PenTool, Users, Music, Play, Sparkles, ChevronRight, Layers, Mic, Scroll } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { HAUSA_SECTIONS, HAUSA_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface HausaSyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const HausaSyllabusView: React.FC<HausaSyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? HAUSA_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Koyi Harshen <span className="text-emerald-600">Hausa</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Master the Hausa Language, Culture, and Literature.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
            {HAUSA_SECTIONS.map((section, index) => {
                let Icon = BookOpen;
                let colorClass = "text-emerald-600";
                let bgClass = "bg-emerald-500/5";

                if (section.id.includes('section-1') || section.id.includes('section-2')) { Icon = PenTool; colorClass = "text-blue-600"; bgClass = "bg-blue-500/5"; }
                if (section.id.includes('section-3') || section.id.includes('section-4')) { Icon = Users; colorClass = "text-amber-600"; bgClass = "bg-amber-500/5"; }
                if (section.id.includes('section-5')) { Icon = Scroll; colorClass = "text-purple-600"; bgClass = "bg-purple-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    if (activeSection.id.includes('section-1') || activeSection.id.includes('section-2')) { themeColor = "text-blue-600"; bgTheme = "bg-blue-50"; }
    if (activeSection.id.includes('section-3') || activeSection.id.includes('section-4')) { themeColor = "text-amber-600"; bgTheme = "bg-amber-50"; }
    if (activeSection.id.includes('section-5')) { themeColor = "text-purple-600"; bgTheme = "bg-purple-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your Hausa session.
                </p>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={HAUSA_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                Hausa <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-emerald-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-emerald-100 max-w-full text-left">
                    ...
                    <span>Comprehensive coverage of Hausa Language and Culture.</span>
                </div>
            </div>
        )}
      </div>
...
export default HausaSyllabusView;
