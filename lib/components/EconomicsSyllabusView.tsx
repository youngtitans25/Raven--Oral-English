...
import { ArrowLeft, BookOpen, TrendingUp, BarChart2, Globe, Play, Sparkles, ChevronRight, Layers, Coins } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { ECONOMICS_SECTIONS, ECONOMICS_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface EconomicsSyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const EconomicsSyllabusView: React.FC<EconomicsSyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? ECONOMICS_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Understand the <span className="text-emerald-700">Economy</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                From micro theories of the firm to macro policies and the Nigerian economy.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
            {ECONOMICS_SECTIONS.map((section, index) => {
                let Icon = BookOpen;
                let colorClass = "text-emerald-700";
                let bgClass = "bg-emerald-500/5";

                if (section.id.includes('section-2')) { Icon = TrendingUp; colorClass = "text-blue-700"; bgClass = "bg-blue-500/5"; }
                if (section.id.includes('section-3')) { Icon = BarChart2; colorClass = "text-indigo-700"; bgClass = "bg-indigo-500/5"; }
                if (section.id.includes('section-4')) { Icon = Coins; colorClass = "text-amber-700"; bgClass = "bg-amber-500/5"; }
                if (section.id.includes('section-5')) { Icon = Globe; colorClass = "text-teal-700"; bgClass = "bg-teal-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    if (activeSection.id.includes('section-2')) { themeColor = "text-blue-700"; bgTheme = "bg-blue-50"; }
    if (activeSection.id.includes('section-3')) { themeColor = "text-indigo-700"; bgTheme = "bg-indigo-50"; }
    if (activeSection.id.includes('section-4')) { themeColor = "text-amber-700"; bgTheme = "bg-amber-50"; }
    if (activeSection.id.includes('section-5')) { themeColor = "text-teal-700"; bgTheme = "bg-teal-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic to start your Economics session.
                </p>
            </div>
                    ...
                    >
                        <div className="flex flex-row sm:flex-col h-full text-slate-900">
                             ...
                             <div className="w-1/3 sm:w-full h-auto sm:h-40 overflow-hidden relative shrink-0">
                                <ImageWithFallback 
                                    src={'https://images.unsplash.com/photo-1611974765270-ca1258634369?q=80&w=1000&auto=format&fit=crop'} 
                                    alt={module.title}
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={ECONOMICS_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        <Button variant="ghost" onClick={handleBack} className="text-slate-600 gap-2 pl-0 hover:bg-transparent hover:text-emerald-700 text-sm">
            <ArrowLeft className="w-4 h-4 md:w-5 md:h-5" /> 
            {selectedSectionId ? 'Back to Sections' : 'Back to Dashboard'}
        </Button>
        <div className="flex items-center gap-2 text-slate-800">
            <Layers className="w-4 h-4 md:w-5 md:h-5 text-emerald-700" />
            <span className="font-display font-bold text-sm md:text-lg">
                Economics <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-emerald-700">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-emerald-100 max-w-full text-left">
                    ...
                    <span>Master wealth creation, markets, and policy.</span>
                </div>
            </div>
        )}
      </div>
...
export default EconomicsSyllabusView;
