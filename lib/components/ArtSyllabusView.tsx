...
import { ART_SECTIONS, ART_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface ArtSyllabusViewProps {
  onBack: () => void;
...
const ArtSyllabusView: React.FC<ArtSyllabusViewProps> = ({ onBack, onStartSection }) => {
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Unleash your <span className="text-purple-600">Creativity</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Explore the world of Art: Classification, History, Techniques, and Appreciation.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto">
            {ART_SECTIONS.map((section, index) => {
                let Icon = Palette;
                let colorClass = "text-purple-600";
                let bgClass = "bg-purple-500/5";

                if (section.id.includes('section-b')) { Icon = Globe; colorClass = "text-fuchsia-600"; bgClass = "bg-fuchsia-500/5"; }
                if (section.id.includes('section-c')) { Icon = Paintbrush; colorClass = "text-violet-600"; bgClass = "bg-violet-500/5"; }
                if (section.id.includes('section-d')) { Icon = Eye; colorClass = "text-indigo-600"; bgClass = "bg-indigo-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    let themeColor = "text-purple-600";
    let bgTheme = "bg-purple-50";
    
    if (activeSection.id.includes('section-b')) { themeColor = "text-fuchsia-600"; bgTheme = "bg-fuchsia-50"; }
    if (activeSection.id.includes('section-c')) { themeColor = "text-violet-600"; bgTheme = "bg-violet-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your Art session.
                </p>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={ART_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        <Button variant="ghost" onClick={handleBack} className="text-slate-600 gap-2 pl-0 hover:bg-transparent hover:text-purple-600 text-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                Art <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-purple-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-purple-50 text-purple-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-purple-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Master the principles of Visual, Performing and Literary Arts.</span>
                </div>
            </div>
        )}
      </div>
...
export default ArtSyllabusView;
