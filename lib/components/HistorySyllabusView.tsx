...
const HistorySyllabusView: React.FC<HistorySyllabusViewProps> = ({ onBack, onStartSection }) => {
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Walk through <span className="text-amber-700">Time</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Explore Nigerian and African history from early civilizations to modern nation-building.
            </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
            {HISTORY_SECTIONS.map((section, index) => {
                ...
                if (section.id.includes('section-a')) { Icon = Landmark; colorClass = "text-orange-700"; bgClass = "bg-orange-500/5"; }
                if (section.id.includes('section-b')) { Icon = Sword; colorClass = "text-red-700"; bgClass = "bg-red-500/5"; }
                if (section.id.includes('section-c')) { Icon = Flag; colorClass = "text-green-700"; bgClass = "bg-green-500/5"; }
                if (section.id.includes('section-d')) { Icon = Globe; colorClass = "text-blue-700"; bgClass = "bg-blue-500/5"; }
                
                // Part 2
                if (section.id.includes('part2-section-a')) { Icon = BookOpen; colorClass = "text-purple-700"; bgClass = "bg-purple-500/5"; }
                if (section.id.includes('part2-section-b')) { Icon = Map; colorClass = "text-indigo-700"; bgClass = "bg-indigo-500/5"; }
                if (section.id.includes('part2-section-c')) { Icon = Scroll; colorClass = "text-teal-700"; bgClass = "bg-teal-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    if (activeSection.id.includes('section-a')) { themeColor = "text-orange-700"; bgTheme = "bg-orange-50"; }
    if (activeSection.id.includes('section-b')) { themeColor = "text-red-700"; bgTheme = "bg-red-50"; }
    if (activeSection.id.includes('section-c')) { themeColor = "text-green-700"; bgTheme = "bg-green-50"; }
    if (activeSection.id.includes('section-d')) { themeColor = "text-blue-700"; bgTheme = "bg-blue-50"; }
    
    // Part 2
    if (activeSection.id.includes('part2-section-a')) { themeColor = "text-purple-700"; bgTheme = "bg-purple-50"; }
    if (activeSection.id.includes('part2-section-b')) { themeColor = "text-indigo-700"; bgTheme = "bg-indigo-50"; }
    if (activeSection.id.includes('part2-section-c')) { themeColor = "text-teal-700"; bgTheme = "bg-teal-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your History session.
                </p>
            </div>
                    ...
                    >
                        <div className="flex flex-row sm:flex-col h-full text-slate-900">
                             ...
                             <div className="w-1/3 sm:w-full h-auto sm:h-40 overflow-hidden relative shrink-0">
                                <ImageWithFallback 
                                    src={'https://images.unsplash.com/photo-1599401738722-e42b6b0b005e?q=80&w=1000&auto=format&fit=crop'} 
                                    alt={module.title}
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            <Layers className="w-4 h-4 md:w-5 md:h-5 text-amber-700" />
            <span className="font-display font-bold text-sm md:text-lg">
                History <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-amber-700">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-amber-50 text-amber-800 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-amber-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Explore key historical events and nation-building in Africa.</span>
                </div>
            </div>
        )}
      </div>
...
export default HistorySyllabusView;