...
import { ArrowLeft, BookOpen, Calculator, PenTool, PieChart, TrendingUp, Play, Sparkles, ChevronRight, Layers, BarChart2 } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { MATHEMATICS_SECTIONS, MATH_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface MathematicsSyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const MathematicsSyllabusView: React.FC<MathematicsSyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? MATHEMATICS_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Calculate your <span className="text-indigo-600">Success</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Master Number Bases, Algebra, Geometry, Calculus, and Statistics.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
            {MATHEMATICS_SECTIONS.map((section, index) => {
                let Icon = Calculator;
                let colorClass = "text-indigo-600";
                let bgClass = "bg-indigo-500/5";

                if (section.id.includes('section-1')) { Icon = Calculator; colorClass = "text-blue-600"; bgClass = "bg-blue-500/5"; }
                if (section.id.includes('section-2')) { Icon = PenTool; colorClass = "text-purple-600"; bgClass = "bg-purple-500/5"; }
                if (section.id.includes('section-3')) { Icon = Layers; colorClass = "text-emerald-600"; bgClass = "bg-emerald-500/5"; }
                if (section.id.includes('section-4')) { Icon = TrendingUp; colorClass = "text-amber-600"; bgClass = "bg-amber-500/5"; }
                if (section.id.includes('section-5')) { Icon = PieChart; colorClass = "text-rose-600"; bgClass = "bg-rose-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    let bgTheme = "bg-indigo-50";
    
    if (activeSection.id.includes('section-1')) { themeColor = "text-blue-600"; bgTheme = "bg-blue-50"; }
    if (activeSection.id.includes('section-2')) { themeColor = "text-purple-600"; bgTheme = "bg-purple-50"; }
    if (activeSection.id.includes('section-3')) { themeColor = "text-emerald-600"; bgTheme = "bg-emerald-50"; }
    if (activeSection.id.includes('section-4')) { themeColor = "text-amber-600"; bgTheme = "bg-amber-50"; }
    if (activeSection.id.includes('section-5')) { themeColor = "text-rose-600"; bgTheme = "bg-rose-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to preview objectives and start your Math session.
                </p>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={MATH_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                Mathematics <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-indigo-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-indigo-50 text-indigo-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-indigo-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Comprehensive coverage from Algebra to Calculus.</span>
                </div>
            </div>
        )}
      </div>
...
export default MathematicsSyllabusView;
