...
import { ArrowLeft, BookOpen, Moon, Scroll, Users, Play, Sparkles, ChevronRight, Layers, GraduationCap } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { IRS_SECTIONS, IRS_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface IRSSyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const IRSSyllabusView: React.FC<IRSSyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? IRS_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Seek <span className="text-emerald-600">Knowledge</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Comprehensive study of the Qur'an, Hadith, Fiqh, and Islamic History.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-5xl mx-auto">
            {IRS_SECTIONS.map((section, index) => {
                let Icon = BookOpen;
                let colorClass = "text-emerald-600";
                let bgClass = "bg-emerald-500/5";

                if (section.id.includes('section-2')) { Icon = Moon; colorClass = "text-indigo-600"; bgClass = "bg-indigo-500/5"; }
                if (section.id.includes('section-3')) { Icon = Scroll; colorClass = "text-amber-600"; bgClass = "bg-amber-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    if (activeSection.id.includes('section-3')) { themeColor = "text-amber-600"; bgTheme = "bg-amber-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your session.
                </p>
            </div>
                    ...
                    >
                        <div className="flex flex-row sm:flex-col h-full text-slate-900">
                             ...
                             <div className="w-1/3 sm:w-full h-auto sm:h-40 overflow-hidden relative shrink-0">
                                <ImageWithFallback 
                                    src={'https://images.unsplash.com/photo-1585036156171-4053915a1c7a?q=80&w=1000&auto=format&fit=crop'} 
                                    alt={module.title}
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={IRS_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                Islamic Studies <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-emerald-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-emerald-100 max-w-full text-left">
                    ...
                    <span>Deepen your understanding of spiritual, moral, and historical traditions.</span>
                </div>
            </div>
        )}
      </div>
...
export default IRSSyllabusView;
