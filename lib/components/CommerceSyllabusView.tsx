...
const CommerceSyllabusView: React.FC<CommerceSyllabusViewProps> = ({ onBack, onStartSection }) => {
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Master the world of <span className="text-indigo-600">Business</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Explore Trade, Production, Finance, Management, and the Business Environment.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
            {COMMERCE_SECTIONS.map((section, index) => {
                let Icon = Briefcase;
                let colorClass = "text-indigo-600";
                let bgClass = "bg-indigo-500/5";

                if (section.id.includes('trade')) { Icon = ShoppingBag; colorClass = "text-blue-600"; bgClass = "bg-blue-500/5"; }
                if (section.id.includes('support')) { Icon = Truck; colorClass = "text-orange-600"; bgClass = "bg-orange-500/5"; }
                if (section.id.includes('finance')) { Icon = DollarSign; colorClass = "text-emerald-600"; bgClass = "bg-emerald-500/5"; }
                if (section.id.includes('manage')) { Icon = BarChart; colorClass = "text-purple-600"; bgClass = "bg-purple-500/5"; }
                if (section.id.includes('legal')) { Icon = Globe; colorClass = "text-slate-600"; bgClass = "bg-slate-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    if (activeSection.id.includes('trade')) { themeColor = "text-blue-600"; bgTheme = "bg-blue-50"; }
    if (activeSection.id.includes('support')) { themeColor = "text-orange-600"; bgTheme = "bg-orange-50"; }
    if (activeSection.id.includes('finance')) { themeColor = "text-emerald-600"; bgTheme = "bg-emerald-50"; }
    if (activeSection.id.includes('manage')) { themeColor = "text-purple-600"; bgTheme = "bg-purple-50"; }
    if (activeSection.id.includes('legal')) { themeColor = "text-slate-600"; bgTheme = "bg-slate-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic to start your Commerce session.
                </p>
            </div>
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                Commerce <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-indigo-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-indigo-50 text-indigo-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-indigo-100 max-w-full text-left">
                    ...
                    <span>Understand business, trade, and economic systems.</span>
                </div>
            </div>
        )}
      </div>
...
export default CommerceSyllabusView;
