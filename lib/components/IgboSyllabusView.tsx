...
import { ArrowLeft, BookOpen, PenTool, Users, Play, Sparkles, ChevronRight, Layers, Mic, Scroll, Globe } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { IGBO_SECTIONS, IGBO_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface IgboSyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const IgboSyllabusView: React.FC<IgboSyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? IGBO_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Mụta Asụsụ <span className="text-red-600">Igbo</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Master the Igbo Language, Literature, Customs, and Traditions.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto">
            {IGBO_SECTIONS.map((section, index) => {
                let Icon = BookOpen;
                let colorClass = "text-red-600";
                let bgClass = "bg-red-500/5";

                if (section.id.includes('section-a')) { Icon = PenTool; colorClass = "text-blue-600"; bgClass = "bg-blue-500/5"; }
                if (section.id.includes('section-b')) { Icon = Scroll; colorClass = "text-purple-600"; bgClass = "bg-purple-500/5"; }
                if (section.id.includes('section-c')) { Icon = Users; colorClass = "text-amber-600"; bgClass = "bg-amber-500/5"; }
                if (section.id.includes('section-d')) { Icon = Globe; colorClass = "text-green-600"; bgClass = "bg-green-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    let bgTheme = "bg-red-50";
    
    if (activeSection.id.includes('section-a')) { themeColor = "text-blue-600"; bgTheme = "bg-blue-50"; }
    if (activeSection.id.includes('section-b')) { themeColor = "text-purple-600"; bgTheme = "bg-purple-50"; }
    if (activeSection.id.includes('section-c')) { themeColor = "text-amber-600"; bgTheme = "bg-amber-50"; }
    if (activeSection.id.includes('section-d')) { themeColor = "text-green-600"; bgTheme = "bg-green-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic below to start your Igbo session.
                </p>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={IGBO_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        <Button variant="ghost" onClick={handleBack} className="text-slate-600 gap-2 pl-0 hover:bg-transparent hover:text-red-600 text-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                Igbo <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-red-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-red-50 text-red-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-red-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Comprehensive coverage of Igbo Language, Literature, and Customs.</span>
                </div>
            </div>
        )}
      </div>
...
export default IgboSyllabusView;
