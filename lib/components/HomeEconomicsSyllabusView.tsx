...
import { ArrowLeft, BookOpen, Home, Utensils, Scissors, Play, Sparkles, ChevronRight, Layers } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { HOME_ECONOMICS_SECTIONS, HOME_ECONOMICS_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface HomeEconomicsSyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const HomeEconomicsSyllabusView: React.FC<HomeEconomicsSyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? HOME_ECONOMICS_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Master the <span className="text-pink-600">Home</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                The JAMB Home Economics syllabus covers four vital pillars of daily living.
                Select a section below to explore detailed topics.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            {HOME_ECONOMICS_SECTIONS.map((section, index) => {
                let Icon = BookOpen;
                let colorClass = "text-emerald-600";
                let bgClass = "bg-emerald-500/5";

                if (section.id.includes('section-b')) { Icon = Home; colorClass = "text-blue-600"; bgClass = "bg-blue-500/5"; }
                if (section.id.includes('section-c')) { Icon = Utensils; colorClass = "text-orange-600"; bgClass = "bg-orange-500/5"; }
                if (section.id.includes('section-d')) { Icon = Scissors; colorClass = "text-purple-600"; bgClass = "bg-purple-500/5"; }
                ...
                return (
                    ...
                    >
                        ...
                        <Card className="flex-1 flex flex-col md:flex-row overflow-hidden border-0 shadow-lg group-hover:shadow-2xl transition-all duration-300 rounded-3xl h-full">
                            <div className="h-48 md:h-auto md:w-2/5 overflow-hidden relative shrink-0">
                                <ImageWithFallback 
                                    src={section.image} 
                                    alt={section.title}
  ...
  const renderDetailView = () => {
    ...
    if (activeSection.id.includes('section-b')) { themeColor = "text-blue-600"; bgTheme = "bg-blue-50"; }
    if (activeSection.id.includes('section-c')) { themeColor = "text-orange-600"; bgTheme = "bg-orange-50"; }
    if (activeSection.id.includes('section-d')) { themeColor = "text-purple-600"; bgTheme = "bg-purple-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic to start your interactive Home Economics session.
                </p>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={HOME_ECONOMICS_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            <Layers className="w-4 h-4 md:w-5 md:h-5 text-pink-600" />
            <span className="font-display font-bold text-sm md:text-lg">
                Home Economics <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-pink-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-pink-50 text-pink-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-pink-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Master the art and science of home management.</span>
                </div>
            </div>
        )}
      </div>
...
export default HomeEconomicsSyllabusView;
