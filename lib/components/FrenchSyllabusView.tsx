...
import { ArrowLeft, BookOpen, Languages, MessageCircle, Mic, Globe, Play, Sparkles, ChevronRight, Layers } from 'lucide-react';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { FRENCH_SECTIONS, FRENCH_INSTRUCTION } from '../coaches';
import { ImageWithFallback } from './ui/ImageWithFallback';
import { SyllabusPreviewModal } from './ui/SyllabusPreviewModal';

interface FrenchSyllabusViewProps {
  onBack: () => void;
  onStartSection: (sectionId: string) => void;
}

const FrenchSyllabusView: React.FC<FrenchSyllabusViewProps> = ({ onBack, onStartSection }) => {
  ...
  const activeSection = selectedSectionId 
    ? FRENCH_SECTIONS.find(s => s.id === selectedSectionId) 
    : null;
    ...
    <div className="animate-fade-in">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
            <h1 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-2 md:mb-4">
                Parlez <span className="text-blue-600">Français</span>.
            </h1>
            <p className="text-slate-600 text-sm md:text-lg">
                Master grammar, comprehension, oral skills, and culture.
            </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
            {FRENCH_SECTIONS.map((section, index) => {
                let Icon = BookOpen;
                let colorClass = "text-blue-600";
                let bgClass = "bg-blue-500/5";

                if (section.id.includes('section-2')) { Icon = Languages; colorClass = "text-indigo-600"; bgClass = "bg-indigo-500/5"; }
                if (section.id.includes('section-3')) { Icon = Layers; colorClass = "text-purple-600"; bgClass = "bg-purple-500/5"; }
                if (section.id.includes('section-4')) { Icon = MessageCircle; colorClass = "text-pink-600"; bgClass = "bg-pink-500/5"; }
                if (section.id.includes('section-5')) { Icon = Mic; colorClass = "text-amber-600"; bgClass = "bg-amber-500/5"; }
  ...
  const renderDetailView = () => {
    ...
    if (activeSection.id.includes('section-3')) { themeColor = "text-purple-600"; bgTheme = "bg-purple-50"; }
    if (activeSection.id.includes('section-4')) { themeColor = "text-pink-600"; bgTheme = "bg-pink-50"; }
    if (activeSection.id.includes('section-5')) { themeColor = "text-amber-600"; bgTheme = "bg-amber-50"; }
        ...
        <div className="animate-slide-up">
             <div className="text-center mb-8 md:mb-10 max-w-3xl mx-auto">
                ...
                <p className="text-slate-600 text-sm md:text-base">
                    Select a topic to start your French session.
                </p>
            </div>
                    ...
                    >
                        <div className="flex flex-row sm:flex-col h-full text-slate-900">
                             ...
                             <div className="w-1/3 sm:w-full h-auto sm:h-40 overflow-hidden relative shrink-0">
                                <ImageWithFallback 
                                    src={'https://images.unsplash.com/photo-1550340499-a6c603072b20?q=80&w=1000&auto=format&fit=crop'} 
                                    alt={module.title}
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {previewTopicId && activePreviewModule && (
          <SyllabusPreviewModal 
            ...
            instructionGenerator={FRENCH_INSTRUCTION}
          />
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between shadow-sm">
        <Button variant="ghost" onClick={handleBack} className="text-slate-600 gap-2 pl-0 hover:bg-transparent hover:text-blue-600 text-sm">
        ...
        <div className="flex items-center gap-2 text-slate-800">
            ...
            <span className="font-display font-bold text-sm md:text-lg">
                French <span className="text-slate-300 mx-1 md:mx-2">/</span> <span className="text-blue-600">{selectedSectionId ? activeSection?.title.split(':')[0] : 'Syllabus'}</span>
            </span>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-12 w-full">
        ...
        {!selectedSectionId && (
            <div className="mt-12 md:mt-16 text-center animate-fade-in">
                <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-3 py-2 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border border-blue-100 max-w-full text-left">
                    <Sparkles className="w-4 h-4 shrink-0" />
                    <span>Master grammar, vocabulary, and culture.</span>
                </div>
            </div>
        )}
      </div>
...
export default FrenchSyllabusView;
